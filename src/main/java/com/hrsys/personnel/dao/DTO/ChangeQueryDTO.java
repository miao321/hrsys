...
public class ChangeQueryDTO {
	...
	private Date applyBegin;	// 申请时间起
	
	@DateTimeFormat(pattern="yyyy/MM/dd")
	private Date applyEnd;		// 申请时间止
	
	@DateTimeFormat(pattern="yyyy/MM/dd")
	private Date agreeBegin;	// 同意时间起
	
	@DateTimeFormat(pattern="yyyy/MM/dd")
	private Date agreeEnd;		// 同意时间止
		...
		Specification<Change> spec = new Specification<Change>() {
			public Predicate toPredicate(Root<Change> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(changeQueryDTO != null && changeQueryDTO.getApplyBegin() != null && changeQueryDTO.getApplyEnd() != null) {
					Predicate p = cb.between(root.get("applyTime").as(Date.class), changeQueryDTO.getApplyBegin(),
							changeQueryDTO.getApplyEnd());
					list.add(p);
				}
				if(changeQueryDTO != null && changeQueryDTO.getAgreeBegin() != null && changeQueryDTO.getAgreeEnd() != null) {
					Predicate p = cb.between(root.get("agreeTime").as(Date.class), changeQueryDTO.getAgreeBegin(),
							changeQueryDTO.getAgreeEnd());
					list.add(p);
				}
